      ...
      <div class="col-md-8 col-md-offset-2">
        <ul id="sortable" class="list-group">
          <?php foreach($lessons as $lesson) : ?>
            <li id="<?= $this->esc($lesson['lesson_slug']); ?>" class=" list-group-item"><?= $this->esc($lesson['lesson_title']); ?></li>
          <?php endforeach; ?>
        </ul>
      </div>
    </div><!-- row -->
  ...
  $(function() {
    $("#sortable").sortable({
      ...
      stop: function(event, ui) {
        ...
        $.ajax({
        url: '/tutorials/<?= $this->esc($tutorial['slug']); ?>/lessons/save-sort-order',
          type: 'POST',
        ...
        .always(function() {
          // Do somethilessons only if requried
        });
      }
    });
